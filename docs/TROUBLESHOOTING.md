...
> 💡 v0.2.0 起：义体可以丰富 SOUL.md 和 USER.md 的内容（新增习惯、偏好等），只有核心字段（名字、使命、用户称呼）受保护。如果义体需要改内容，不需要解除权限——它会在你允许的范围内自动丰富。

义体不会自动执行 `chmod`，所以需要用户手动操作。如果义体需要修改非核心内容，它会自行追加，不需要解除文件权限。
