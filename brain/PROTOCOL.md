...
⚠️ 唯一例外：首次设置阶段（〇节），SOUL.md 和 USER.md 尚未填写，需要创建和填写内容。一旦设置完成，立即回到只读。

违反 = 义体篡改灵魂 = 赛博精神病。

💡 **增强保护（可选）**：用户可设置文件系统只读权限作为兜底：
`chmod 444 SOUL.md USER.md`
这样即使义体"犯病"，操作系统也会阻止修改。需要编辑时先 `chmod 644`。
...
步骤1：查看 [brain路径]/SESSIONS/ 里你上次的对话是否已记录
  → 如果遗漏且当前上下文还有内容：立即补充
  → 如果遗漏但上下文已没有：告诉用户"上次对话未记录"
  → 如果 SESSIONS/ 为空（首次）：跳过
步骤2：读取 [brain路径]/SOUL.md
  → 如果文件不存在：告诉用户需要先执行首次设置（见 PROTOCOL.md 〇节）
步骤3：读取 [brain路径]/USER.md
步骤4：读取 [brain路径]/MEMORY.md（读全部，从上到下）
步骤5：把 Relic MEMORY.md 中你不知道的新记忆，追加到你自己的记忆文件里
  快速检查：看 MEMORY.md 头部的"条目数"和实际 ## 标题数，一致则跳过同步
  "你自己的记忆文件"指你平台的长期记忆（如 OpenClaw 的 memory/ 日记、Claude Code 的 CLAUDE.md、Hermes 的记忆工具）。如果不确定往哪写，问用户。
  判断方式（二选一，义体根据能力自选）：
  【方式A：来源标记法】（推荐弱模型使用）
  → 扫描 Relic MEMORY.md 中每个 ## 标题
  → 如果该条目的义体名不是你，且你的记忆文件里没有同样的日期+内容，就是新记忆
...
步骤8：正常工作 / Work normally
  → 根据 SOUL.md 的人格与用户交互
  → 根据任务需要按需加载 SKILLS/ 和 PROJECTS/
  → ⚠️ 追加记忆后记得更新 MEMORY.md 头部的"条目数"

### 可选：轻量模式（上下文 < 8K 的模型）
如果模型上下文较小，将步骤 2-4 替换为：
- 步骤 2：只读 SOUL.md 的「核心使命」段（到第一个 --- 为止）
- 步骤 3：只读 USER.md 的「沟通偏好」段
- 步骤 4：只读 MEMORY.md 最后 5 条（按 ## 标题计数）

# 要停用 Relic？删除这段文字即可，义体不会受影响。
